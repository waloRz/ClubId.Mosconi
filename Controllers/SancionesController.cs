    ...
    public async Task <IActionResult> GenerarBoletin(int id)
{
    ...
    public async Task<IActionResult> BuscarJugadores(string term)
    {
        ...
        var results = await _context.Jugadores
            // Busca por nombre, apellido o DNI que contenga el término
            .Where(j => j.Nombre.Contains(term) || j.Apellido.Contains(term) || j.Dni.Contains(term))
            .Take(10) // Limita resultados para mejor rendimiento
            .Select(j => new
            {
                id = j.Idjugador, // Necesario para el valor del select
